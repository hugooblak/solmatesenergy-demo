...
add_filter(
	...
	function ( $exporters ) {
		$exporters['solmatesenergy-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'solmatesenergy-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'solmatesenergy-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'solmatesenergy-leads',
						'group_label' => __( 'Quote requests', 'solmatesenergy-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['solmatesenergy-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'solmatesenergy-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
