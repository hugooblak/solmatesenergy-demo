...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'solmatesenergy-leads' ),
					'singular_name' => __( 'Lead', 'solmatesenergy-leads' ),
					'edit_item'     => __( 'Förfrågan', 'solmatesenergy-leads' ),
					'search_items'  => __( 'Search leads', 'solmatesenergy-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'solmatesenergy-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'solmatesenergy-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'solmatesenergy-leads' ),
			'npl_phone'    => __( 'Phone', 'solmatesenergy-leads' ),
			'npl_service'  => __( 'Needs', 'solmatesenergy-leads' ),
			'npl_timeline' => __( 'When', 'solmatesenergy-leads' ),
			'npl_zip'      => __( 'ZIP', 'solmatesenergy-leads' ),
			'npl_source'   => __( 'Source', 'solmatesenergy-leads' ),
			'npl_status'   => __( 'Status', 'solmatesenergy-leads' ),
			'date'         => __( 'Received', 'solmatesenergy-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'solmatesenergy-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'solmatesenergy-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'solmatesenergy-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'solmatesenergy-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'solmatesenergy-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'solmatesenergy-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'solmatesenergy-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'solmatesenergy-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'solmatesenergy-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'solmatesenergy-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'solmatesenergy-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'solmatesenergy-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
