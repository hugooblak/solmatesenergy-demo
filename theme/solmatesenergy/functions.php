<?php
/**
 * Solmates Energy theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Solmates Energy Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through so_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'solmatesenergy', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'solmatesenergy' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'solmatesenergy' ) ) );

		register_block_pattern_category( 'solmatesenergy-sections', array( 'label' => __( 'Solmates Energy sections', 'solmatesenergy' ) ) );
		register_block_pattern_category( 'solmatesenergy-pages', array( 'label' => __( 'Solmates Energy page layouts', 'solmatesenergy' ) ) );
	}
);
