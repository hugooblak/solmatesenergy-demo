 ...
 * or photo. They call so_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Solmates Energy.
 *
 ...
 * @return string
 */
function so_sample_note() {
	return so_lead( 'ar_riktig' ) ? '' : ' <span class="so-sample">(platshållare)</span>';
}
 ...
 * @return string File name for so_lead_img().
 */
function so_lead_bild( $i ) {
	$bilder = (array) so_lead( 'bilder', array() );
 ...
 * @return string
 */
function so_ring_label() {
	$namn = so_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, so_lead( 'telefon' ) ) : 'Ring ' . so_lead( 'telefon' );
}
